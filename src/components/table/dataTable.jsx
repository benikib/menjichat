function DataTable({ columns, data, actions, getActions }) {
      ...
      <table className="min-w-full text-left divide-y divide-gray-200">
        <thead className="bg-gray-100">
          <tr className="bg-gray-2 text-left dark:bg-meta-4">
            ...
            {(actions || getActions) && <th className="px-6 py-3 text-sm font-medium text-gray-700">Actions</th>}
          </tr>
        </thead>
        {data?.length > 0 ? <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row, index) => {
            ...
            <tr key={row.id} className="hover:bg-gray-50">
              <td className="border-b border-[#eee] py-8 px-4">{index + 1}</td>
              {columns.map((col) =>
                col.key !== "id" ? (
              ...
              {rowActions && (
                <td className=" flex gap-2 border-b border-[#eee] py-8 px-4">
                  {rowActions.map((action, idx) => (
                    <button
                      key={idx}
                      onClick={() => action.callback(row)}
                      className="text-grey-500 p-2 rounded-full hover:opacity-80"
                    >
                      {action.label === "Voir"
        ...
        </tbody> : <tbody><tr><td colSpan="8" className="text-center">
          Aucun bien trouvé
        </td></tr></tbody>}
